...
fun DualSimApp(repository: TelephonyRepository) {
    ...
    LaunchedEffect(monitoring) {
        if (monitoring) {
            monitoredSample()
            while (monitoring) { delay(10_000); if (monitoring) monitoredSample() }
        }
    }
    Scaffold(bottomBar = { NavigationBar(Modifier.navigationBarsPadding()) { Tab.values().forEach { item -> NavigationBarItem(selected = tab == item, onClick = { tab = item }, icon = { Text(item.label.take(1)) }, label = { Text(item.label) }) } } }) { padding ->
        Column(Modifier.fillMaxSize().padding(padding).padding(16.dp)) {
            when (tab) {
                Tab.HOME -> HomeScreen(state, refreshing, ::manualRefresh)
                Tab.MONITOR -> MonitorScreen(state, monitoring, { monitoring = true }, { monitoring = false })
...
@Composable private fun HomeScreen(state: MonitorState, refreshing: Boolean, refresh: () -> Unit) {
